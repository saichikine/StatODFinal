function [sys] = SRP3BDynamicsParamsSunInVarCR(t, X, CRTimeVec, params)

% State is [R, V, Rs1, Rs2, Rs3, br3, br3dot, muSun, omegaE, Cr]

% Get CR as a function of time (linearly interpolate between the known
% values
tprevIndex = find(CRTimeVec(:,2)<=t,1,'last');
if tprevIndex==length(CRTimeVec)
    CR = CRTimeVec(end,1);
else
    tprev = CRTimeVec(tprevIndex,2);
    tnext = CRTimeVec(tprevIndex+1,2);
    interpX = [tprev tnext];
    interpTimes = [CRTimeVec(tprevIndex,1) CRTimeVec(tprevIndex+1,1)];
    CR = interp1(interpX, interpTimes, t);
%     CR = CRTimeVec(tprevIndex,1);
end
