function UKFStruct = UKF_DMC(initStuff, measurements, stationStates, noiseStuff, dynamicsFunction, measurementFunction, ...
    ...
    while i<=N
        ...
        stationState = stationStates(:,stationIndex,i);

        YMat = NaN(yLen, 2*L+1);
        for j = 1:(2*L+1)
            YMat(:,j) = (measurementFunction(chi(:,j), stationState));
        end
        ...
        postfitResiduals(:,i,stationIndex) = y-measurementFunction(Xhat, stationState);
